...
function updatePackageVersion(packageName, newVersion) {
  ...
  function updateNestedDependencies(dependencies) {
    for (const [depName, depDetails] of Object.entries(dependencies || {})) {
      if (depName === packageName && depDetails.version !== newVersion) {
        ...
        console.log(
          chalk.green(
            `✔ Updated ${packageName} to version ${newVersion} in nested dependencies.`
          )
        );
      }
  ...
  try {
    ...
    ["dependencies", "devDependencies"].forEach((depType) => {
      if (packageJson[depType] && packageJson[depType][packageName]) {
        if (packageJson[depType][packageName] === newVersion) {
          console.log(
            chalk.green(
              `✔ ${packageName} is already at version ${newVersion} in ${depType}.`
            )
          );
        } else {
          ...
          console.log(
            chalk.green(
              `✔ Updated ${packageName} to ${newVersion} in ${depType}.`
            )
          );
        }
      }
    ...
    if (updated || nestedUpdated) {
      ...
      console.log(
        chalk.yellow(
          `⚠️  Skipping npm install because ${packageName} was not updated.`
        )
      );
    }
  } catch (err) {
...
function createBranchAndPush(packageName, newVersion) {
  try {
    ...
    try {
      ...
      console.log(
        chalk.yellow(`⚠️  Branch ${branchName} already exists locally.`)
      );
      executeCommand(
    ...
    executeCommand(
      'git commit -m "chore: update package version" || echo "No changes to commit."',
      "Failed to commit changes."
    );

    console.log(chalk.blue("Pushing changes to remote..."));
    executeCommand(
      `git push --set-upstream origin ${branchName} || git push origin ${branchName} --force`,
